...
import CoreGraphics
  ...
  protocol  SpinWheelDelegate: NSObjectProtocol {
    func wheelDidChangeValue(_ newValue: Int)
     //func lastAnimation() ->CABasicAnimation
 ...
 public enum LandingPostion:Int {
    case top = 270
    case left = 180
    case right = 360
    case bottom = 90
...
open class SpinWheel :UIControl{
    ...
    func drawWheel() {
        ...
        for i in 0..<numberOfSections {
            drawPath(start, and: end, with: UIColor.systemGray, andName: "\((numberOfSections - i))", andIndex: i)
            start += angleSize
            end += angleSize
        ...
        labelsView = SpinWheelText(frame: CGRect(x: 0, y: -wheelRadius / 2, width: bounds.width, height: bounds.height + wheelRadius ))
    ...
    func getSelectedSector(_ section: Int) {
        if dataSource?.numberOfSections() == 3 {
            selectedSection = getSectorByValue(section-1)
        } else {
